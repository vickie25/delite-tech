...
export const adminPost = async <T>(path: string, data: any): Promise<T> => {
  ...
  if (!response.ok) throw new Error(body.message || "Failed to post data");
  return body as T;
};
...
export const adminPut = async <T>(path: string, data: any): Promise<T> => {
  ...
  const body = await response.json();
  if (!response.ok) throw new Error(body.message || "Failed to update data");
  return body as T;
};
...
export const adminDelete = async <T>(path: string): Promise<T> => {
  ...
  const body = await response.json();
  if (!response.ok) throw new Error(body.message || "Failed to delete data");
  return body as T;
};
